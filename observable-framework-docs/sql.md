---
url: "https://observablehq.com/framework/sql"
title: "SQL  | Observable Framework"
---

1. [SQL code blocks](https://observablehq.com/framework/sql#sql-code-blocks)
2. [SQL literals](https://observablehq.com/framework/sql#sql-literals)

# [SQL](https://observablehq.com/framework/sql\#sql) [Added in v1.2.0](https://github.com/observablehq/framework/releases/tag/v1.2.0 "Added in v1.2.0")

This page covers client-side SQL using DuckDB. To run a SQL query on a remote database such as PostgreSQL or Snowflake, use a [data loader](https://observablehq.com/framework/data-loaders).

Framework includes built-in support for client-side SQL powered by [DuckDB](https://observablehq.com/framework/lib/duckdb). You can use SQL to query data from [CSV](https://observablehq.com/framework/lib/csv), [TSV](https://observablehq.com/framework/lib/csv), [JSON](https://observablehq.com/framework/files#json), [Apache Arrow](https://observablehq.com/framework/lib/arrow), [Apache Parquet](https://observablehq.com/framework/lib/arrow#apache-parquet), and DuckDB database files, which can either be static or generated by [data loaders](https://observablehq.com/framework/data-loaders).

To use SQL, first register the desired tables in the page’s [front matter](https://observablehq.com/framework/markdown#front-matter) using the **sql** option. Each key is a table name, and each value is the path to the corresponding data file. For example, to register a table named `gaia` from a Parquet file:
...
To load externally-hosted data, use a full URL: [Added in 1.4.0](https://github.com/observablehq/framework/releases/tag/v1.4.0 "Added in 1.4.0")
...
For performance and reliability, we recommend using local files rather than loading data from external servers at runtime. You can use a [data loader](https://observablehq.com/framework/data-loaders) to take a snapshot of a remote data during build if needed.

You can also register tables via code (say to have sources that are defined dynamically via user input) by defining the `sql` symbol with [DuckDBClient.sql](https://observablehq.com/framework/lib/duckdb). To register [DuckDB extensions](https://observablehq.com/framework/lib/duckdb#extensions), use the [**duckdb** config option](https://observablehq.com/framework/config#duckdb).

## [SQL code blocks](https://observablehq.com/framework/sql\#sql-code-blocks)
...
This returns an array of 10 rows as an [Apache Arrow](https://observablehq.com/framework/lib/arrow) table, inspected here:
...
## [SQL literals](https://observablehq.com/framework/sql\#sql-literals)
